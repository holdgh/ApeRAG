...
from aperag.tasks.collection import init_collection_task, delete_collection_task
from aperag.schema.view_models import Collection, CollectionList, SearchTestResult, SearchTestResultItem, SearchTestResultList
...
from aperag.db.ops import PagedQuery
...
async def update_collection(user: str, collection_id: str, collection: view_models.CollectionUpdate) -> view_models.Collection:
...
async def create_search_test(user: str, collection_id: str, data: view_models.SearchTestRequest) -> view_models.SearchTestResult:
    collection = await query_collection(user, collection_id)
    if not collection:
    ...
    for idx, doc in enumerate(docs):
        items.append(SearchTestResultItem(
            rank=idx+1,
            score=doc.score,
            content=doc.text,
