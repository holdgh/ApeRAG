...
from aperag.chat.sse.base import BaseConsumer, ChatRequest, MessageProcessor, logger, BaseFormatter
...
class FrontendFormatter(BaseFormatter):
    ...
    @staticmethod
    def format_stream_end(msg_id: str, references: List[str] = None,
...
class FrontendConsumer(BaseConsumer):
    ...
    async def _handle(self, body):
        ...
        try:
            ...
            if request.stream:
                await self._send_streaming_response(request.msg_id, processor.process_message(request.message, request.msg_id))
            else:
                await self._send_complete_response(request.msg_id, processor.process_message(request.message, request.msg_id))
