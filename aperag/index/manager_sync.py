...
"""
Synchronous version of DocumentIndexManager for use in Celery tasks.
"""
...
class DocumentIndexManagerSync:
    """Simple manager for document index specs (sync version for Celery)"""

    def create_or_update_document_indexes(
        self, session: Session, document_id: str, index_types: Optional[List[DocumentIndexType]] = None
    ):
        """
        Create or update index records for a document (called when document is created or index is updated)
...
# Global instance
document_index_manager_sync = DocumentIndexManagerSync()
