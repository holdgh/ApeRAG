...
#   timestamp: 2025-07-10T14:39:17+00:00
...
class MergeSuggestionsRequest(BaseModel):
    ...
    max_suggestions: Optional[conint(ge=1, le=50)] = Field(
        10, description='Maximum number of merge suggestions to return', example=10
    )
...
class MergeSuggestionEntity(BaseModel):
    """
    Entity information in merge suggestion
    """

    entity_id: str = Field(..., description='Entity ID', example='墨香居')
    entity_name: str = Field(..., description='Entity name', example='墨香居')
    entity_type: str = Field(..., description='Entity type', example='ORGANIZATION')
    description: str = Field(
        ..., description='Entity description', example='这条老巷子里唯一的旧书店'
    )
    degree: Optional[int] = Field(
        None, description='Node degree (number of connections)', example=8
    )
...
class MergeSuggestion(BaseModel):
    ...
    entities: list[MergeSuggestionEntity] = Field(
        ..., description='List of entities suggested for merging', min_items=2
    )
