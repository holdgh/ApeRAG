...
image-authing: build-requirements version
	docker buildx build -t $(REGISTRY)/apecloud/kubechat:$(VERSION)-authing --platform $(BUILDX_PLATFORM) $(BUILDX_ARGS) --push -f ./Dockerfile  .
