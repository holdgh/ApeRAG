...
from deeprag.utils.constant import KEY_BOT_ID, KEY_CHAT_ID, KEY_USER_ID
from deeprag.utils.utils import extract_bot_and_chat_id, extract_web_bot_and_chat_id


async def bot_consumer_router(scope, receive, send):
    from deeprag.db.models import BotType, CollectionType
    from deeprag.db.ops import query_bot, query_chat
    ...
    if bot.type == BotType.KNOWLEDGE:
        if collection.type == CollectionType.DOCUMENT:
            ...
            if settings.CHAT_CONSUMER_IMPLEMENTATION == "document-qa":
                from deeprag.chat.websocket.document_qa_consumer import DocumentQAConsumer
                return await DocumentQAConsumer.as_asgi()(scope, receive, send)
            elif settings.CHAT_CONSUMER_IMPLEMENTATION == "fake":
                from deeprag.chat.websocket.fake_consumer import FakeConsumer
                return await FakeConsumer.as_asgi()(scope, receive, send)
            else:
                from deeprag.chat.websocket.embedding_consumer import EmbeddingConsumer
                return await EmbeddingConsumer.as_asgi()(scope, receive, send)
        elif collection.type == CollectionType.DATABASE:
            from deeprag.chat.websocket.text_2_sql_consumer import Text2SQLConsumer
        ...
        from deeprag.chat.websocket.common_consumer import CommonConsumer
...
async def web_bot_consumer_router(scope, receive, send):
    from deeprag.db.models import BotType
    from deeprag.db.ops import query_bot, query_web_chat
