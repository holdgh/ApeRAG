...
from deeprag.chat.utils import check_quota_usage, get_async_redis_client, get_sync_redis_client, manage_quota_usage
from deeprag.db.models import Chat, ChatPeer
from deeprag.db.ops import query_bot, query_chat_by_peer, query_user_quota
from deeprag.pipeline.knowledge_pipeline import KnowledgePipeline
from deeprag.utils.weixin.client import WeixinClient
from deeprag.utils.weixin.WXBizMsgCrypt import WXBizMsgCrypt
...
async def weixin_text_response(client, user, bot, query, msg_id):
    ...
    try:
        ...
        await client.send_message("DeepRAG 正在解答中，请稍候......", user)
...
async def weixin_card_response(client, user, bot, query, msg_id):
    ...
    try:
        ...
        _, response_code = await client.send_card("DeepRAG 正在解答中，请稍候......", user, task_id)
...
async def weixin_feedback_response(client, user, bot, key, response_code, task_id):
    ...
    await deeprag.chat.message.feedback_message(bot.user, chat.id, msg_id.decode(), upvote, downvote, "")
...
async def officialaccount_callback(request, user, bot_id, signature, timestamp, nonce, openid, encrypt_type, msg_signature):
    ...
    if xml_tree.find("MsgType") is not None and xml_tree.find("MsgType").text == "text":
        ...
        if redis_client.exists(query_record):
            ...
            response = f"DeepRAG 已收到问题，请发送{msg_id}获取答案"
