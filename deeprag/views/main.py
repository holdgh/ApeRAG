...
import deeprag.chat.message
from config import settings
from config.celery import app
from deeprag.apps import QuotaType
from deeprag.auth.validator import GlobalHTTPAuth
...
from deeprag.llm.base import Predictor
from deeprag.llm.prompts import (
...
from deeprag.tasks.collection import delete_collection_task, init_collection_task
from deeprag.tasks.crawl_web import crawl_domain
from deeprag.tasks.index import (
...
from deeprag.tasks.scan import delete_sync_documents_cron_job, update_sync_documents_cron_job
from deeprag.tasks.sync_documents_task import get_sync_progress, sync_documents
from deeprag.utils.request import get_urls, get_user
from deeprag.views.utils import (
    add_ssl_file,
    fail,
    query_chat_messages,
    success,
...
async def feedback_message(request, bot_id, chat_id, message_id, msg_in: MessageFeedbackIn):
    ...
    if chat is None:
        return fail(HTTPStatus.NOT_FOUND, "Chat not found")
    feedback = await deeprag.chat.message.feedback_message(chat.user, chat_id, message_id, msg_in.upvote,
                                                            msg_in.downvote,
...
def dashboard(request):
    ...
    return render(request, 'deeprag/dashboard.html', context)
