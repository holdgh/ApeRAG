...
class CustomLoadDocumentTask(Task):
    def on_success(self, retval, task_id, args, kwargs):
        ...
        return super(CustomLoadDocumentTask, self).on_success(
            retval, task_id, args, kwargs
        )

    def on_failure(self, exc, task_id, args, kwargs, einfo):
        # print("task failed!")
        return super(CustomLoadDocumentTask, self).on_failure(
            exc, task_id, args, kwargs, einfo
        )

    def after_return(self, status, retval, task_id, args, kwargs, einfo):
        print(retval)
        return super(CustomLoadDocumentTask, self).after_return(
...
def add_index_for_document(document_id, file_path):
    ...
    try:
        loader = LocalPathEmbedding(
            input_files=[file_path],
            vector_store_adaptor=get_vector_db_connector(
                collection=generate_qdrant_collection_id(
                    user=document.user, collection=document.collection.id
                )
            ),
        )
        ...
        logger.debug(
            f"add qdrant points: {document.relate_ids} for document {file_path}"
        )
...
def remove_index(document_id):
    ...
    try:
        logger.debug(
            f"remove qdrant points: {document.relate_ids} for document {document.file}"
        )
...
def update_index(document_id, file_path):
    ...
    try:
        ...
        document.relate_ids = ",".join(ids)
        logger.debug(
            f"update qdrant points: {document.relate_ids} for document {file_path}"
        )
