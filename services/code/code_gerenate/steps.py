...
from services.code.code_gerenate.AI import AI
from services.code.code_gerenate.chat_to_files import to_files
from services.code.code_gerenate.storage import DBs
...
def simple_gen(ai: AI, dbs: DBs) -> List[dict]:
    ...
    message = ai.start(setup_sys_prompt(dbs), get_prompt(dbs), step_name=curr_fn())
    to_files(message[-1]["content"], dbs.workspace)
    return message
