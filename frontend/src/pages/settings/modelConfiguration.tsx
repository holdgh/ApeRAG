...
export default () => {
  ...
  }, [editingProvider, providerForm, setLoading, fetchConfiguration, formatMessage]);
  ...
  const handleDeleteProvider = useCallback(async (provider: LlmProvider) => {
    ...
    if (confirmed) {
      ...
      try {
        ...
        message.success(formatMessage({ id: 'model.provider.delete.success' }));
        await fetchConfiguration();
      } catch (error) {
  ...
  const handleAddModel = useCallback(() => {
    ...
    if (currentProvider) {
      ...
      modelForm.setFieldValue('custom_llm_provider', currentProvider.completion_dialect || 'openai');
    }
    setModalView('add');
  ...
  }, [editingModel, modelForm, setLoading, fetchConfiguration, formatMessage, currentProvider]);
  ...
  const providerColumns: TableProps<LlmProvider>['columns'] = [
    {
      ...
      width: 120,
      render: (name: string) => (
        <Text code style={{ fontSize: '12px' }}>{name}</Text>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          >
            {formatMessage({ id: 'model.provider.manage' })}
          </Button>
  ...
  const modalModelColumns: TableProps<LlmProviderModel>['columns'] = [
    ...
    },
    {
      ...
      render: (tags: string[]) => (
        <Space size={4} wrap>
          {tags?.map(tag => (
            <Tag key={tag} color="processing" style={{ fontSize: '11px', margin: '1px' }}>
              {tag}
            </Tag>
          ))}
        </Space>
    ...
    <div style={{ padding: '0 24px' }}>
      ...
      <Card
        title={
          <Space>
            <SettingOutlined />
            <span>{formatMessage({ id: 'model.provider.title' })}</span>
          </Space>
        }
        extra={
          <Button
      ...
      >
        ...
        >
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                ...
                rules={[{ 
                  required: true, 
                  message: formatMessage({ id: 'model.provider.name.required' })
                }]}
              >
                <Input 
                  disabled={!!editingProvider} 
                  placeholder={formatMessage({ id: 'model.provider.name.placeholder' })}
            ...
            <Col span={12}>
              <Form.Item
                ...
                rules={[{ 
                  required: true, 
                  message: formatMessage({ id: 'model.provider.label.required' })
                }]}
              >
                <Input 
                  placeholder={formatMessage({ id: 'model.provider.label.placeholder' })}
                  style={{ borderRadius: '6px' }}
                />
              </Form.Item>
            </Col>
          ...
          <Form.Item
            ...
            rules={[{ 
              required: true, 
              message: formatMessage({ id: 'model.provider.base_url.required' })
            }]}
          >
            <Input 
              placeholder={formatMessage({ id: 'model.provider.base_url.placeholder' })}
              style={{ borderRadius: '6px' }}
            />
          </Form.Item>
          ...
          <Row gutter={16}>
            <Col span={8}>
              <Form.Item 
                ...
                label={formatMessage({ id: 'model.provider.completion_dialect' })}
                rules={[{ 
                  required: true, 
                  message: formatMessage({ id: 'model.provider.completion_dialect.required' })
                }]}
              >
                <Input 
                  placeholder={formatMessage({ id: 'model.provider.completion_dialect.placeholder' })}
                  style={{ borderRadius: '6px' }}
                />
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item 
                ...
                label={formatMessage({ id: 'model.provider.embedding_dialect' })}
                rules={[{ 
                  required: true, 
                  message: formatMessage({ id: 'model.provider.embedding_dialect.required' })
                }]}
              >
                <Input 
                  placeholder={formatMessage({ id: 'model.provider.embedding_dialect.placeholder' })}
                  style={{ borderRadius: '6px' }}
                />
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item 
                ...
                rules={[{ 
                  required: true, 
                  message: formatMessage({ id: 'model.provider.rerank_dialect.required' })
                }]}
              >
                <Input 
                  placeholder={formatMessage({ id: 'model.provider.rerank_dialect.placeholder' })}
                  style={{ borderRadius: '6px' }}
                />
              </Form.Item>
            </Col>
          ...
          <Form.Item 
            ...
            label={formatMessage({ id: 'model.provider.allow_custom_base_url' })}
          >
            <Switch />
          </Form.Item>
          ...
          >
            <Input.TextArea 
              placeholder={formatMessage({ id: 'model.provider.extra_config.placeholder' })}
              rows={3} 
              style={{ borderRadius: '6px' }}
            />
          </Form.Item>
      ...
      <Modal
        title={
          currentProvider ? formatMessage(
            { id: 'model.management.title' },
            { provider: currentProvider.label }
          ) : formatMessage({ id: 'model.management.title' })
        }
            ...
            >
              <Row gutter={16}>
                <Col span={12}>
                  <Form.Item
                    ...
                    rules={[{ 
                      required: true, 
                      message: formatMessage({ id: 'model.api_type.required' })
                    }]}
                  >
                    <Select
                      placeholder={formatMessage({ id: 'model.api_type.required' })}
                ...
                <Col span={12}>
                  <Form.Item
                    ...
                    rules={[{ 
                      required: true, 
                      message: formatMessage({ id: 'model.model_name.required' })
                    }]}
                  >
                    <Input 
                      placeholder={formatMessage({ id: 'model.model_name.placeholder' })}
                      disabled={!!editingModel} 
                      style={{ borderRadius: '6px' }}
                    />
                  </Form.Item>
              ...
              <Row gutter={16}>
                <Col span={12}>
                  ...
                  >
                    <InputNumber 
                      placeholder={formatMessage({ id: 'model.max_tokens.placeholder' })}
                      min={1} 
                ...
                <Col span={12}>
                  ...
                  >
                    <Select
                      mode="tags"
                      placeholder={formatMessage({ id: 'model.tags.placeholder' })}
                      tokenSeparators={[',']}
                      style={{ width: '100%', borderRadius: '6px' }}
                    />
                  </Form.Item>
