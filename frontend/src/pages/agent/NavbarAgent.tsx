...
import { Navbar, NavbarBody } from '@/components';
...
export const NavbarAgent = () => {
  ...
  const { bot, chats, bots, getBot, getBots, setBot, setChats, getChats } =
    useModel('bot');
  ...
  const onDeleteChat = useCallback(
    async (item: Chat) => {
      ...
      if (res.status === 204) {
        ...
        if (_.isEmpty(data)) {
          history.push(`/agent/${botId}/chats`);
        } else if (chatId === item.id) {
          history.push(`/agent/${botId}/chats/${data[0]?.id}`);
        }
      }
    },
    [botId, chatId, chats],
    ...
    return [
      {
        ...
        children: (chats || []).map((item) => {
          const path = `/agent/${botId}/chats/${item.id}`;
          return {
  ...
  useEffect(() => {
    if (location.pathname === '/agent' && bots) {
      const agentBot = bots.find((b) => b.type === 'agent');
      if (agentBot) {
        history.replace(`/agent/${agentBot.id}/chats`);
      }
    }
  }, [bots, location.pathname]);
